...
var CaptureResponseTests = {
	...
	getReconciliationIdentifier_getsTheLastReconciliationIdentifier_whenThereIsOnlyOne : function()
	{
		...
		Assert.equals('705e3fc6-99a0-4a07-9a0d-0b4b2b0ba811', captureResponse.getReconciliationIdentifier());
	},
	...
	getReconciliationIdentifier_getsTheLastReconciliationIdentifier_with3Elements : function()
	{
		...
		Assert.equals('4', captureResponse.getReconciliationIdentifier());
	},

	getRegisteredAddress_readFromCustomerInfo : function()
	{
		var captureResponse = new CaptureResponse(
		...
		Assert.equals('first name',registeredAddress.firstName);
		Assert.equals('last name',registeredAddress.lastName);
		Assert.equals('address',registeredAddress.address);
		Assert.equals('city',registeredAddress.city);
		Assert.equals('region',registeredAddress.region);
		Assert.equals('country',registeredAddress.country);
		Assert.equals('postal code',registeredAddress.postalCode);
	},

	getRegisteredAddress_ifNoRegisteredAddress_returnNull : function()
	{
		var captureResponse = new CaptureResponse(
			{Body:{Transactions:{Transaction:{
				CustomerInfo:{
	...
	getRegisteredAddress_ifNoCustomerInfo_returnNull : function()
	{
		var captureResponse = new CaptureResponse(
			{Body:{Transactions:{Transaction:{
			}}}}
